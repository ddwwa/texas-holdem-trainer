#!/usr/bin/env tsx
/**
 * Test showdown with some players folding
 */
...
console.log(colors.bright + colors.cyan + '\n=== SHOWDOWN TEST (WITH FOLDS) ===\n' + colors.reset);

const game = new GameManager();
game.startNewHand();

// Some players fold, some stay
let actions = 0;
let playersFolded = 0;
while (game.getCurrentActor() && actions < 100) {
  ...
  // Fold some players (AI 3, AI 5, AI 6)
  if ((actor.name === 'AI 3' || actor.name === 'AI 5' || actor.name === 'AI 6') && playersFolded < 3) {
...
const playersFoldedList = state.players.filter(p => p.hasFolded);
...
// Show players who folded
if (playersFoldedList.length > 0) {
  console.log(colors.dim + `\n❌ Folded: ${playersFoldedList.map(p => p.name).join(', ')}` + colors.reset);
}

// Show all remaining players' cards and hands
...
console.log(colors.green + `\n✓ Showdown with ${playersFoldedList.length} folds working!\n` + colors.reset);
